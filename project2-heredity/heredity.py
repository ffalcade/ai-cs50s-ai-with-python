...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    """
    # Helper: probability a parent passes the gene to a child
    def pass_prob(parent_genes):
        m = PROBS["mutation"]
        if parent_genes == 2:
            return 1 - m
        if parent_genes == 1:
    ...
    for person, info in people.items():
        # Determine this person's gene count
        ...
        # If no parents listed, use unconditional gene probability
        mother = info["mother"]
        father = info["father"]
        if mother is None and father is None:
            gene_prob = PROBS["gene"][genes]
        else:
            # Compute from parents' passing probabilities
            ...
            if genes == 2:
                ...
                gene_prob = p_m * (1 - p_f) + p_f * (1 - p_m)
            else:  # genes == 0
                gene_prob = (1 - p_m) * (1 - p_f)

        # Trait conditional probability
